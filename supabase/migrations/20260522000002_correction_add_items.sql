-- ============================================================
-- Расширение apply_correction_batch: добавление НОВЫХ тем/задач
-- ============================================================
-- Реальный кейс: организация прислала замечание не «правка формулировки»,
-- а «вот ещё одно поручение, которое потеряли». В режиме правки утверждённого
-- протокола нужно уметь добавить новую задачу/тему в тот же batch правок:
-- одна транзакция, один event 'protocol_correction', единый audit-trail.
--
-- Расширение существующей функции (см. 20260507000006_protocol_corrections.sql):
--   kind='topic' / 'task'         — UPDATE существующих (как раньше)
--   kind='add_task' / 'add_topic' — INSERT новой записи со статусом 'open'/'approved',
--                                   meta полями corrected_*, revisions=[{action:'created_by_correction'}]
--
-- См. WIKI 10_Алгоритм_собрания.md → запись 22.05.2026.
...
begin
    ...
    loop
        ...
        -- ─── INSERT новой задачи (по замечанию) ───────────
            ...
            -- Генерация кода: max(NN) + 1 по существующим tasks.code с префиксом
            -- v_meeting_code-ЗАД-
            select coalesce(max(
        ...
        -- ─── INSERT новой темы (по замечанию) ─────────────
    ...
    if v_n_topics + v_n_tasks + v_n_topics_added + v_n_tasks_added = 0 then
    ...
    -- Схема events (после миграции 20260508000003_events_simplify): нет
    -- is_planned/is_manual/fact_date. Источник = 'protocol' (derived_source),
    -- date_start = today; date_computed проставит триггер.
...
comment on function apply_correction_batch(jsonb) is
    'Атомарно применяет пакет правок к утверждённому собранию: UPDATE существующих тем/задач (kind=topic/task) + INSERT новых (kind=add_topic/add_task) с revisions/correction_* + INSERT event protocol_correction + entity_links.';
