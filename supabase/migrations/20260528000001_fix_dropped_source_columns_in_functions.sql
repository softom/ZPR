-- ============================================================
-- FIX: функции, INSERT'ящие в удалённые колонки tasks
-- ============================================================
-- Контекст: миграция 20260526000001_tasks_lifecycle_links.sql отбросила
-- legacy-колонки tasks: source_protocol / source_meeting_date /
-- source_meeting_path / migrated_from / object_codes (переход на
-- meeting_id-based связи). Но две функции, созданные ДО неё, продолжали
-- ссылаться на source_protocol/source_meeting_date/source_meeting_path
-- в INSERT INTO tasks — на этапе исполнения это даёт:
--   "column source_protocol of relation tasks does not exist"
--
-- Поломанные пути:
--   • convert_topic_to_task   — кнопка «перенести Тему → Задачу» (Секция 6)
--   • apply_correction_batch  — kind='add_task' (кнопка «+ Добавить задачу»
--                                в режиме правки по замечанию)
--
-- Здесь — CREATE OR REPLACE обеих функций БЕЗ ссылок на удалённые колонки.
-- Связь задачи с собранием теперь полностью держится на tasks.meeting_id,
-- который обе функции и так проставляют. Никаких других изменений в логике.
...
begin
    ...
    -- Базовый префикс кода задачи (v_source — только для генерации кода,
    -- в колонку tasks больше НЕ пишется: связь идёт через meeting_id)
...
comment on function convert_topic_to_task(uuid, jsonb) is
    'Атомарный перенос темы в задачу: INSERT preliminary task + UPDATE topic.status=removed. Связь — через tasks.meeting_id (legacy source_* колонки удалены).';
...
comment on function apply_correction_batch(jsonb) is
    'Атомарно применяет пакет правок к утверждённому собранию: UPDATE (kind=topic/task), INSERT (kind=add_topic/add_task), логическое удаление (kind=remove_topic/remove_task) + event protocol_correction + entity_links. Связь задач — через meeting_id (legacy source_* колонки удалены).';
