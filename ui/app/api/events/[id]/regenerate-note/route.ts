...
export async function POST(
  ...
  const { data: ev, error: evErr } = await supabaseAdmin
    .from('events')
    .select('id, event_type, title, note, date_end, object_ids')
    .eq('id', id)
    .maybeSingle()
...
function buildPrompt(opts: {
  ...
  return `Ты — помощник руководителя строительного проекта «Золотые пески России». Перепиши запись события в журнале проекта — корректно, по-деловому, оставляя только суть.

═══ КОНТЕКСТ (для твоего понимания — НЕ переписывать его в текст) ═══
...
В карточке события рядом с текстом уже отображаются как СВЯЗИ:
  • объекты (chip-теги с кодами и именами)
  • юр.лица / подрядчики (chip-теги)
  • дата события (отдельным полем)

Поэтому в тексте note:
- ❗ НЕ упоминай имена объектов («Гостиница на 350 номеров», «106_ГОСТИНИЦА_350»). Если в исходнике они были — убери.
- ❗ НЕ упоминай названия подрядчиков / юр.лиц («ООО МЛА+», «по запросу MLA+»). Если в исходнике были — убери.
- ❗ НЕ дублируй дату события (${dateCtx}). Внутренние даты по содержанию (срок поставки, дата документа) — оставляй.
- ❗ НЕ начинай предложение с даты.

Пиши ТОЛЬКО суть события: что произошло, какое действие, какой документ, какой результат. Без «по объекту X», «по запросу Y», «по поручению Z» — это всё есть в связях карточки.

ХОРОШО: «Назначено рабочее совещание для согласования планировочных решений номерного фонда.»
ПЛОХО: «По запросу проектной организации MLA+ назначено рабочее совещание по объекту "Гостиница на 350 номеров" (106_ГОСТИНИЦА_350).»

Для note:
- Сохрани все факты из исходного текста (даты документов, сроки, цифры, решения)
- Деловой нейтральный тон, без эмоций
- Без markdown, без списков
- Не добавляй информации, которой нет в исходном тексте
- Сохрани ссылки и wiki-ссылки (![[...]] и [текст](url)) как есть

Для title:
- Короткое описательное название (а не первая фраза note)
- Без точки в конце
- Без даты в начале
- Без названий объектов и подрядчиков (они и так в связях)
- Не дублирует note
