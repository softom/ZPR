...
3. Ближайшие контрольные точки и срок ТЭП — одной фразой (срок ТЭП — ТОЛЬКО
   если он передан в данных; если нет — тему ТЭП опусти).
4. Изменения ФЗ — одной фразой, если были.

ФОРМУЛИРОВКИ (тон справки для руководства/акционеров):
- Задачи с истёкшим плановым сроком НЕ называй «просрочены» / «срок истёк» /
  «не выполнено в срок» / «с нарушением срока». Пиши нейтрально: они
  **имеют срок исполнения и находятся в работе** (при необходимости — «в
  работе, срок исполнения DD.MM.YYYY»). Не акцентируй внимание на истечении.
- Срок формирования ТЭП упоминай ТОЛЬКО если он есть в данных. Если срока ТЭП
  в данных НЕТ — НЕ пиши «не задан» / «не определён» / «в системе не указан» /
  «отсутствует» — просто опусти эту тему целиком.
