...
// Шесть контентных полей секции отчёта по объекту:
//   project_movement       (1)   — абзац
//   achievements           (2.1) — описание (абзац)
//   achievements_list      (2.2) — основные пункты (Markdown-список)
//   next_period_tasks      (3.1) — описание (абзац)
//   next_period_tasks_list (3.2) — основные пункты (Markdown-список)
//   risks                  (4)   — абзац
...
const FIELD_PROMPTS: Record<ReportField, string> = {
  project_movement:
`"project_movement" — Существующее движение проекта на этом объекте.
2-4 предложения. Снимок текущего состояния: что делается прямо сейчас, на каком
этапе работы, какая активность в рабочих собраниях. Допустимо в первой фразе
упомянуть числа (например «На объекте идёт работа по N активным задачам»).
Если активности нет — «На объекте в текущем периоде активных работ не зафиксировано.»`,
...
// Если fields не задан — возвращает все 6.
export async function generateSections(
  ctx: ReportContext,
  fields?: ReportField[],
): Promise<GeneratedSections> {
  if (!POLZA_API_KEY) throw new Error('POLZA_API_KEY не задан в env')
  const requested = fields && fields.length > 0 ? fields : (REPORT_FIELDS as readonly ReportField[]).slice()
...
- Не упоминай коды задач (ПРОТ-…) — это внутренние идентификаторы
...
function formatEvents(items: EventRow[]): string {
  ...
  return items.map((e, i) => {
    ...
    if (e.fact_date) parts.push(`[факт ${e.fact_date}]`)
    else if (e.date_computed) parts.push(`[план ${e.date_computed}]`)
