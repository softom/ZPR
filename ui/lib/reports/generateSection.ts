...
const FIELD_PROMPTS: Record<ReportField, string> = {
  // ─── Weekly v3 поля ───────────────────────────────────────────
  weekly_done_brief:
`"weekly_done_brief" — Список «✓ Выполнено / зафиксировано за период».
Markdown-список из 2-7 пунктов через "* " (звёздочка + пробел). Каждый пункт:
- начинается с даты в формате "DD.MM" — БЕЗ жирного выделения, НЕ оборачивай дату
  в звёздочки ** (никакого markdown-форматирования внутри пункта)
- далее " — " и короткое утверждение что произошло (1 предложение)
- если это закрытая задача — упомяни в утверждении (без кода задачи)
Если за период на объекте ничего не зафиксировано — верни строку
"* За отчётный период активности по объекту не зафиксировано."
Пример пункта:
"* 30.04 — получен документ «Инструкция по дизайну для номеров Club» от МЛА+."`,

  weekly_topics_brief:
`"weekly_topics_brief" — Краткое обобщение тем собраний за период
(если собрания были).
...
Если за период собраний не было — верни пустую строку "".
Пример:
...
принят рабочий график до 10.06.2026 с контрольными точками 07.05 / 21.05 / 28.05."`,
  ...
  weekly_upcoming_brief:
`"weekly_upcoming_brief" — Список «🔜 Предстоит». ПРОСТО перечень задач.
Markdown-список из 2-7 пунктов через "* ". ЧТО включать:
1. Плановые задачи и события наступающего периода.
2. **ОБЯЗАТЕЛЬНО — текущие задачи В РАБОТЕ (open/in_progress) из списка «ТЕКУЩИЕ
   АКТИВНЫЕ ЗАДАЧИ» и из «ВАЖНЫЕ СОБЫТИЯ С АКТИВНЫМ FOLLOWUP».** Они продолжаются в
   наступающем периоде, поэтому входят в «Предстоит» — ДАЖЕ если у них нет срока.
Каждый пункт — ТОЛЬКО суть задачи (короткое утверждение, без кода). НЕ добавляй:
- префиксы «без срока» и «до DD.MM»;
- любые пометки про сроки, особенно истёкшие («срок истёк DD.MM», «задача не закрыта», «просрочена»);
- технические пометки в скобках (исполнитель, приоритет — «(ТЗ-ЮГ, _medium_)» и т.п.).
Возвращай "* На наступающий период задач не запланировано." ТОЛЬКО если реально
нет ни плановых задач/событий, ни задач в работе.
Пример пункта:
"* Подготовить письмо с перечнем замечаний по материалам ООО «Визавир»."`,
...
Тон нейтрально-деловой, БЕЗ пессимизма.
...
- **НЕ начинать с отрицания/отсутствия.** Если за период нет закрытых задач —
  НЕ писать «За период закрытых задач не зафиксировано» (и любые подобные фразы
  про отсутствие). Просто не упоминай отсутствие закрытого — сразу переходи к тому,
  что В РАБОТЕ.
- **НЕ приводить счётчик просроченных задач** и не называть задачи «просроченными»
  в этом разделе. Срок задачи можно указать нейтрально («срок — DD.MM.YYYY»), но
  без слов «просрочена», «с нарушением срока», «не выполнено в срок».

ПОРЯДОК И СОДЕРЖАНИЕ:
1. Если за период ЕСТЬ закрытые задачи / решённые проблемы — первой фразой назови
   это как факт: «За период закрыты N задач, в том числе…».
2. Если закрытых задач НЕТ — начинай сразу с того, что В РАБОТЕ: «В работе N задач: …»
   / «Ведётся работа по …» — с сутью ключевых задач и их сроками (нейтрально),
   без счётчика просрочек.
3. Затем — собрания / ключевые решения за период (факт, без оценки результата).

Допустимо упоминать числа закрытых и задач в работе — это факты.

Если на объекте за период нет ни закрытых, ни активных задач, ни событий —
«На объекте в отчётном периоде зафиксированных работ нет.»`,
...
function buildPrompt(ctx: ReportContext, fields: ReportField[]): string {
  ...
  const periodLabel = period.type === 'week'
    ? `период ${ddmmyyyy(period.start)} — ${ddmmyyyy(period.end)}`
...
- **РАЗЛИЧАЙ «закрыто за период» (resolved/done) и «в работе сейчас» (active).**
  Это разные категории — не смешивай их в одном утверждении. Если закрытое есть —
  оно идёт первым; если закрытого нет — НЕ упоминай его отсутствие, начинай с «в работе».
